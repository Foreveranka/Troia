...
describe('engine escalate (indeterminate loss review)', () => {
  it('durably flags loss and quarantines the seq without any mutation', async () => {
    ...
    const ctx = makePreChargeCtx(h.store); // late allocation: the seq is handed out DURING chargeOk, then quarantined

    // SolvencyReserved --chargeOk--> UsdcSubmitted[allocateSeq, persistInFlight, submitPay]; the single observe
    // returns the indeterminate verdict, which escalates (there is no core event to advance on).
    ...
    // the seq allocated at chargeOk is QUARANTINED: still active, never burned, never released, never reused
    expect(r.ctx.activeSeq).not.toBeNull(); // allocateSeq ran (the seq exists) before the indeterminate observe
    const seqs = h.store.sequences as SequenceAllocator;
    expect(seqs.statusOf(BigInt(r.ctx.activeSeq as string))).toBe('active');
