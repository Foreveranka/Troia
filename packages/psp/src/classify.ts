...
export function classifyIyzicoResult(raw: RawIyzicoResult, op: PspOp): IyzicoClass {
  ...
  // NOTE (live-smoke lesson): a success-envelope with paymentStatus 'FAILURE' (e.g. a failed 3DS attempt) is
  // deliberately NOT classified as a charge FAILURE. iyzico's hosted checkout form lets the customer RETRY on the
  // SAME token after a failed attempt, so paymentStatus 'FAILURE' is NOT terminal — a later attempt may reach
  // 'SUCCESS'. A live Troy run proved this: a retrieve read 'FAILURE', then (after the customer retried the 3DS)
  // 'SUCCESS'/mdStatus 1 with the money CAPTURED. Fail-cleaning on the first 'FAILURE' would strand that later
  // capture (charged TRY, order closed, no USDC, no void = LOSS). So it stays UNKNOWN — the order remains open and
  // a retry-success settles it. A DEFINITIVE terminal decline still comes via the status:'failure' + errorCode
  // whitelist path above; the sequence-leak for a genuinely abandoned order is solved by late seq allocation.
