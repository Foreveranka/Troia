...
describe('response projectors — carry happy-path fields, never fabricate a missing one', () => {
  it('projectCheckoutFormInit extracts token/content/conversationId; missing -> malformed', () => {
    const ok = projectCheckoutFormInit(
      body({
        status: 'success',
        token: 't',
  ...
  it('projectCheckoutFormResult extracts token/paymentId/paymentStatus/conversationId', () => {
    const ok = projectCheckoutFormResult(
      body({ token: 't', paymentId: 'p1', paymentStatus: 'SUCCESS', conversationId: 'c1' }),
    );
    expect(ok).toEqual({
      kind: 'ok',
      token: 't',
    ...
    expect(projectCheckoutFormResult(body({ token: 't', paymentStatus: 'SUCCESS' })).kind).toBe(
      'malformed',
    );
    ...
    expect(
      projectCheckoutFormResult(
        body({ token: 't', paymentId: '36434664', paymentStatus: 'FAILURE', conversationId: 'c1' }),
      ).kind,
    ).toBe('ok');
  });

  it('projectPayment extracts paymentId/conversationId', () => {
    expect(projectPayment(body({ paymentId: 'p1', conversationId: 'c1' }))).toEqual({
      kind: 'ok',
      paymentId: 'p1',
      conversationId: 'c1',
    });
  ...
  it('transport markers project to malformed (never a fabricated success)', () => {
    ...
    expect(
      projectPayment(body({ paymentId: 123 as unknown as string, conversationId: 'c1' })).kind,
    ).toBe('malformed');
  });
});
